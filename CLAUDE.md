...
Tool for migrating messages between two Telegram accounts. Hosted at <https://tgmigrate.com>. Backend: FastAPI + Telethon. Frontend: React SPA (Vite + DaisyUI/Tailwind).
...
Production runs on GCP e2-micro (`us-west1-b`) behind Cloudflare proxy:

- DNS: Cloudflare, `tgmigrate.com` → VM external IP, proxied (orange cloud)
- Origin: Caddy 2.x with Let's Encrypt TLS, trusting Cloudflare's published edge ranges
- App: single uvicorn worker, multi-user mode, SQLite for sessions/credentials at rest (encrypted)
- Updates: `docker compose pull && docker compose up -d` on the VM (manual; CI publishes to GHCR)

See `scripts/gcp-bootstrap.md` for first-time provisioning.
...
│   ├── cli.py                   # `telegram-migrator` console script
...
│   ├── message_copy.py          # Copy-mode message construction
│   ├── live_forwarder.py        # Real-time message forwarding via Telethon events
│   ├── event_broadcaster.py     # SSE fan-out helper
│   ├── rate_limiter.py          # Token bucket + FloodWait backoff
│   ├── error_strategies.py      # Telegram error classification → retry/fail
│   ├── routes/
│   │   ├── auth.py              # Login/logout (JSON API)
│   │   ├── chats.py             # List dialogs, fetch messages (REST)
│   │   ├── transfer.py          # Transfer jobs (REST + SSE)
...
│   └── static/                  # Frontend build output (dist/) and minimal assets
